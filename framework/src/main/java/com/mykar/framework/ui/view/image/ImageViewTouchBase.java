...
import com.mykar.framework.ui.view.gif.GifView;


abstract class ImageViewTouchBase extends GifView {
    ...
    float mMaxZoom;
    boolean mIsBounce = true;
    boolean mNeedRecycle = false;
...
//        final float durationMils = 500f;
//        float dx = deltaX / durationMils;
//        float dy = deltaY / durationMils;
//        
    ...
    float usedTime;
    float lastUsedTime ;
    protected void center(boolean horizontal, boolean vertical , final float duration) {
    	   if (mBitmapDisplayed.getBitmap() == null) {
    		   Log.i(TAG, "mBitmapDisplayed bitmap is null");
               return;
           }
          ...
          Log.d(TAG, String.format("deltaX bitmapSize:%sX%s, viewSize:%dX%d, rect:%f,%f,%f,%f",
		  ...
		  float deltaY = 0;
           ...
           usedTime =0;
           lastUsedTime =0;
           final float dx = deltaX/duration ; 
           final float dy = deltaY/duration;
           ...
           this.mHandler.post(new Runnable(){
			...
			public void run() {			 
				...
				usedTime =   Math.min(duration, now - startTime);
			   float ddx =	dx* (usedTime - lastUsedTime)  ;
			   float ddy =	dy* (usedTime - lastUsedTime)  ;
...
//        }
    ...
    protected void needRecycle()
    {
    	mNeedRecycle = true;
    	Log.d(TAG, "needRecycle");
    	postInvalidate();
    }
    ...
    protected void doRecycle()
    {
    	mNeedRecycle = true;
    	Log.d(TAG, "doRecycle");
    	if (mBitmapDisplayed != null && mBitmapDisplayed.getBitmap()!= null && !mBitmapDisplayed.getBitmap().isRecycled())
        {
